...
ifeq ($(HAS_CMAKE),0)
  ifeq ($(USE_AEC),1)
    $(error ERROR, Need cmake for AEC support, install cmake or set USE_AEC = 0)
  endif
  ifeq ($(USE_OPENJPEG),1)
    $(error ERROR, Need cmake for OpenJPEG support, install cmake or set USE_OPENJPEG = 0)
  endif
endif
...
ifeq (${COMP_SYS},oneapi_linux)
   ...
   ifeq ($(need_ftn),1)
      wCPPFLAGS+=-DIFORT
      wLDFLAGS+=-lifcore
      wFFLAGS+=-c -O3 -ffast-math -heap-arrays 32 
      FTN_REAL8:="-r8"
   endif
   ifeq ($(USE_OPENMP),1)
      a:=$(shell echo '$Hdefine USE_OPENMP' >> ${CONFIG_H})
...
ifeq ($(USE_OPENJPEG),1)
   ...
   a:=$(shell echo "\#define USE_OPENJPEG \"${ojsrc}\"" >> ${CONFIG_H})
