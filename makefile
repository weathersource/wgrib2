...
# 
#  MAKE_SHARED_LIBRARY: "make lib" will make both shared and static wgrib2 libraries
#      The shared library is for use by pywgrib2 (Python).  In theory, the shared library
#      can be used by programs requiring the wgrib2 library but the makefile has
#      has no versioning of the libwgrib2.so file which makes life difficult.
#      The shared library enables position independent code and is only supported by
#      COMP_SYS=gnu_linux, CC=gcc, FC=gfortran.  Other compilers are not supported.
...
#      endif
   ...
   ifeq ($(MAKE_SHARED_LIB),1)
      a:=$(shell echo "\#define MAKE_SHARED_LIB" >> ${CONFIG_H})
      PIC_option:=-fPIC
      wCPPFLAGS+=-fPIC -g
      wFFLAGS+=-fPIC -g
...
a:=$(shell echo "\#define CPPFLAGS \"${wCPPFLAGS}\"" >> ${CONFIG_H})
a:=$(shell echo "\#define FFLAGS \"${wFFLAGS}\"" >> ${CONFIG_H})
